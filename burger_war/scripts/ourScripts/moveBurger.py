...
class OnigiriRun(object):
	...
	def __init__(self):
		...
		#ROS subscriber
		self.move_base_status_sub = rospy.Subscriber('move_base/result', GoalStatus, self.GoalStatusArrayCallback, queue_size=1)
	...
	def GoalStatusArrayCallback(self, data):
		...
		#uint8 RECALLED        = 8
		#uint8 LOST            = 9
		if len(data.status.status) != 0:
			self.movebase_status = data.status.status

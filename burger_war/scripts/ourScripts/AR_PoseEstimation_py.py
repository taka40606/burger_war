...
class ARPoseEstimation():
	def __init__(self):
		...
		self.enemybot_x_filed_cs = 0 # [m]
		self.enemybot_y_filed_cs = 0 # [m]
		self.enemybot_angle_filed_cs = 0 # [rad]
	...
	def selectMinimamDistId(self, marker_array_x,  marker_array_y, marker_ids):
		return field_marker_id, enemy_marker_id
	...
	def changeMarkerPose2RobotPose(self):
		...
		if self.detected_marker_flag == True:
			for i in range(len(self.detected_ids)):
				...
				if marker_array_number < self.marker_array_num_first_field:
					...
					if enemy_marker_diagonal > diagonal:
						...
						#self.enemybot_x_filed_cs, self.enemybot_y_filed_cs, self.enemybot_angle_filed_cs = self.changeEnemyPose_Mybot2FieldCS(red_state[0], red_state[1], red_state[2], enemy_marker_x, enemy_marker_y, enemy_marker_angle)
						# 実践用
						self.enemybot_x_filed_cs, self.enemybot_y_filed_cs, self.enemybot_angle_filed_cs = self.changeEnemyPose_Mybot2FieldCS(self.mybot_x_odom, self.mybot_y_odom, self.mybot_yaw_odom, enemy_marker_x, enemy_marker_y, enemy_marker_angle)
			...
			if myFunc.isFieldOut(self.enemybot_x_filed_cs, self.enemybot_y_filed_cs) != True:
				self.enemybot_pose_update_flag = True
				
				#print("-------enemy--------")
				#print(self.enemybot_x_filed_cs)
				#print(self.enemybot_y_filed_cs)
				#print(self.enemybot_angle_filed_cs)
	...
	def getMarkNumber(self):
		self.marker_ids = [52, 50, 51, 42, 40, 41, 9, 10, 5, 6, 11, 12, 7, 8, 1, 3, 4, 2]
	...
	def publishEstimatedPose(self):
		...
		if self.enemybot_pose_update_flag == True:
			vec3 = Vector3(x=self.enemybot_x_filed_cs, y=self.enemybot_y_filed_cs, z=self.enemybot_angle_filed_cs)
